...
Dự án đang ở **giai đoạn nền móng**: đã có hạ tầng kỹ thuật (logging, database, xử lý lỗi, validation, rate limit, health check) cùng hai module `user` và `auth` có phân quyền theo role. Các module nghiệp vụ tiếp theo sẽ do team xác định và được xây trên nền này.
...
| Module `user` (CRUD người dùng) | ✅ Có | Chỉ role `ADMIN` truy cập |
| Module `auth` (JWT + refresh token + session) | ✅ Có | Tự đăng ký luôn nhận role `USER` |
| Phân quyền theo role (`@Roles`) | ✅ Có | `/users` yêu cầu `ADMIN`; ADMIN đầu tiên tạo bằng `npm run db:seed:admin` |
| Health check | ✅ Có | `GET /health/live`, `GET /health/ready` (kiểm tra DB); `HEALTHCHECK` trong Dockerfile |
...
│   │   ├── database/               # Drizzle pool/provider, drizzle.config, script migrate, script seed ADMIN
│   │   └── logger/                 # TaggedLoggerAdapter, request id, HTTP access log
│   └── modules/                    # Bounded context
│       ├── user/                   # Quản lý người dùng, hằng số role
│       ├── auth/                   # Đăng ký, đăng nhập, refresh token, session, guard xác thực/phân quyền
│       └── health/                 # Liveness / readiness cho Docker, Kubernetes
├── drizzle/migrations/             # File migration SQL sinh bởi drizzle-kit
├── test/                           # E2E test (Supertest, cần DB thật)
├── postman/                        # Postman collection: Health, Auth, Users
├── docs/                           # Tài liệu
...
| 2026-09-24 | `3ab99f2` | Module `auth`, `UserRepositoryPort`, cột `users.role`, bảng `sessions` |
| (chưa commit) | — | Phân quyền `ADMIN` cho `/users`, khoá role khi tự đăng ký, script `db:seed:admin`, module `health`, Swagger *Authorize* |
...
|---|---|---|---|
| 1 | 🔴 Cao | JWT secret có giá trị mặc định viết cứng; `.env.example` không có biến JWT → dễ chạy production với secret công khai | `src/modules/auth/infrastructure/config/auth.config.ts` |
| 2 | 🟠 Trung bình | Access token (kèm role bên trong) vẫn dùng được sau khi logout, bị hạ quyền hoặc bị khoá, cho tới khi hết hạn (mặc định 15 phút) | `jwt-auth.guard.ts` |
| 3 | 🟠 Trung bình | Tin tuyệt đối header `X-Forwarded-For`; chuỗi IP > 64 ký tự hoặc User-Agent > 500 ký tự làm login/register lỗi 500 | `auth.controller.ts`, `session.schema.ts` |
| 4 | 🟠 Trung bình | CORS mở cho mọi origin | `main.ts` |
| 5 | 🟡 Thấp | Hạn session viết cứng 7 ngày, không theo `REFRESH_EXPIRES_IN_DAYS` | `auth.service.ts` |
| 6 | 🟡 Thấp | Session hết hạn / đã thu hồi không bao giờ bị xoá; `sessions.user_id` chưa có index | `session.schema.ts` |
| 7 | 🟡 Thấp | Mật khẩu Postgres trong `docker-compose.yml` (`password`) khác `.env.example` (`postgres`) | Gốc repo |
| 8 | 🟡 Thấp | Image Docker không chạy được `db:migrate` (thiếu `tsx` và thư mục `drizzle/`) | `Dockerfile` |
| 9 | ⚪ Kiến trúc | Module `auth` import DTO của tầng presentation và hàm static của `UserService` bên module `user` | `auth.service.ts` |

Đã xử lý: `/users` từng public hoàn toàn và `POST /auth/register` từng cho tự chọn role `ADMIN` — xem [SECURITY.md](./SECURITY.md) R1, R2.
...
1. **Bắt buộc JWT secret** (mục 6 #1) trước khi triển khai ra môi trường thật.
2. **Endpoint tự phục vụ cho người dùng** — đổi mật khẩu của chính mình. Hiện chỉ ADMIN sửa được thông tin người dùng qua `/users`.
3. **Chuẩn hoá cấu hình** bằng một config loader có validate (fail-fast khi thiếu biến bắt buộc).
4. **Dọn session** định kỳ và thêm index cho bảng `sessions`.
