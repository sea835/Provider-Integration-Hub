...
describe('TaggedLoggerAdapter', () => {
    const baseConfig: LoggerConfig = { level: 'info', format: 'json', redactKeys: ['password'] };

    beforeEach(() => loggerRegistry.clear());

    function setup(config: Partial<LoggerConfig> = {}) {
        const lines: { level: LogLevel; line: string }[] = [];
        const logger = new TaggedLoggerAdapter({ ...baseConfig, ...config }, [], (level, line) =>
    ...
    it('nối tag qua child()', () => {
        ...
        expect(JSON.parse(lines[0].line)).toMatchObject({ level: 'info', tags: ['APP', 'User', 'UserService'], msg: 'hello' });
    });
    ...
    it('format pretty dạng [TAG][TAG]', () => {
        ...
        expect(lines[0].line).toMatch(/INFO\s+\[APP\]\[User\] hello \{"id":1\}$/);
    });
    ...
    it('redact key nhạy cảm, kể cả lồng nhau', () => {
        ...
        expect(JSON.parse(lines[0].line).meta).toEqual({ user: { email: 'a@b.c', Password: '[REDACTED]' } });
    });

    it('serialize error và gắn requestId từ context', () => {
        const { logger, lines } = setup();
        RequestContext.run({ requestId: 'req-1' }, () => logger.error('boom', new Error('fail')));
